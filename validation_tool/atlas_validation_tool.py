...
#
# For usage instructions and examples, run "python atlas_validation_tool.py --help".
#
# Development Notes:
...
# NOTE: see TODO comments for areas of improvement and future work


# TODO: add f'{normalization_option}' to each plot method in the title to show if there is a normalization_option set and what it is
# TODO: find out if i can autoscale the dist plots (see --mode dist, --htype TH1)
# TODO: add error handling
# TODO: hear audio, --print optional arg that instead prints out the --mode chi2
...
def calculate_chi2(file1: Any, file2: Any, hist_type: str, f_path_type: str, chi2options: str, output_val_dict: Dict[str, Any]) ->Tuple[Any, Dict[str, Any]]:
    ...
    # TODO: May need to remove None to prevent execution halting. Keep it if you want execution to halt   
...
def chi2_test_2d(args: Any, chi2options: str, file1: Any, file2: Any, f_path_type: str, normalization_option: Optional[str] = None, min_stat=1) -> Tuple[Optional[Any], Optional[Any]]:
        ...
        for i in range(hist_2d_n_bins_x):
            for j in range(hist_2d_n_bins_y):
                ...
                if error_squared > 0.000001:
                    ...
                    # TODO: Consider logging this or else it will fill the window with prints and take a long time
                    # print(f"Insufficient statistics in bin ({i},{j}).") 
    ...
    # Handle exceptions
        ...
        logging.error(f"TypeError: {e} - while preparing histograms")
    except Exception as e:
        print("Other error:",e)

    return None  # (makes datapoints invisible) when an error occurs
    # return -9999 # return this value for histograms that had an error while constructing the return value
...
def plot_dist_th1(df: pd.DataFrame, chi2options: str, bins:int=10000, sizex:int=15, sizey:int=9):
    ...
    if "P" in chi2options:
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH1 P-Value freq', color='blue') # marker = ?
        plt.xlabel(f'P-Value')
        plt.title(f'TH1 P-Values:"{chi2options}" Distplot')
    elif "NDF" in chi2options:
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH1 Chi2/NDF Value freq', color='blue') # marker = ?
        plt.xlabel(f'Chi2/NDF Value')
        plt.title(f'TH1 Chi2/NDF:"{chi2options}" Values Distplot')
    else:
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH1 Chi2 Value freq', color='blue') # marker = ?
        plt.xlabel(f'Chi2 Value')
        plt.title(f'TH1 Chi2 Values:"{chi2options}" Distplot')
...
def plot_dist_th2(df: pd.DataFrame, chi2options: str, bins:int=50, sizex:int=15, sizey:int=9):
    ...
    plt.figure(figsize=(sizex,sizey))
    if "P" in chi2options:    
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH2 P-Value freq', color='blue')
        plt.xlabel(f'P-Value')
        plt.title(f'TH2 P-Values:"{chi2options}" Distplot')
    elif "NDF" in chi2options:
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH2 Chi2/NDF Value freq', color='blue')
        plt.xlabel(f'Chi2/NDF Value')
        plt.title(f'TH2 Chi2/NDF Values:"{chi2options}" Distplot')
    else:
        plt.hist(hist_data, bins=bins, alpha=0.7, label=f'TH2 Chi2 Value freq', color='blue')
        plt.xlabel(f'Chi2 Value')
        plt.title(f'TH2 Chi2 Values:"{chi2options}" Distplot')
    plt.ylabel('Frequency')
    plt.legend(loc='upper right')
    plt.show()
...
def plot_dist_tps(df: pd.DataFrame, chi2options: str, bins:int=50, sizex:int=15, sizey:int=9):
    ...
    plt.figure(figsize=(sizex,sizey))
    if "P" in chi2options:
        plt.hist(hist_data, bins=bins, alpha=0.7, color='b', label=f'TProfile P-Value freq')
        plt.xlabel(f'P-value')
        plt.title(f'TProfile P-Values:"{chi2options}" Distplot')
    elif "NDF" in chi2options:
        plt.hist(hist_data, bins=bins, alpha=0.7, color='b', label=f'TProfile Chi2/NDF Value freq')
        plt.xlabel(f'Chi2/NDF Value')
        plt.title(f'TProfile Chi2/NDF Values:"{chi2options}" Distplot')
    else:
        plt.hist(hist_data, bins=bins, alpha=0.7, color='b', label=f'TProfile Chi2 Value freq')
        plt.xlabel(f'Chi2 Value')
...
def plot_chi2_th1s(df_th1: pd.DataFrame, chi2options: str, sizex:int=15, sizey:int=9):
    ...
    if "P" in chi2options:
        ...
        plt.title(f'TH1 P-Values:"{chi2options}" values by hist')
    elif "NDF" in chi2options:
        plt.ylabel(f'Chi2/NDF')
        plt.title(f'TH1 Chi2/NDF:"{chi2options}" values by hist')
    else:
        plt.ylabel(f'Chi2 Value')
        plt.title(f'TH1 Chi2 Values:"{chi2options}" values by hist')
...
def plot_chi2_th2s(df_th2: pd.DataFrame, chi2options: str, sizex:int=15, sizey:int=9):
    ...
    # Plot the possibly normalized output data of the TH2 histograms
    plt.figure(figsize=(sizex,sizey))
    plt.scatter(df_th2['f_name'].values, df_th2['output_vals'].values, marker='o', color='blue')
    if "P" in chi2options:
        plt.ylabel(f'P-Value')
        plt.title(f'TH2 P-Values:"{chi2options}" values by hist')
    elif "NDF" in chi2options:
        plt.ylabel(f'Chi2/NDF')
        plt.title(f'TH2 Chi2/NDF:"{chi2options}" values by hist')
    else:
        plt.ylabel(f'Chi2 Value')
        plt.title(f'TH2 Chi2 Values:"{chi2options}" values by hist')
    plt.xlabel('Hist Name')
    plt.xticks(rotation=90)
    plt.grid(True)
    plt.show()
...
def plot_chi2_tps(df_tp: pd.DataFrame, chi2options: str, sizex:int=15, sizey:int=9):
    ...
    if "P" in chi2options:
        plt.ylabel(f'P-Value')
        plt.title(f'TProfile P-Values:"{chi2options}" values by hist')
    elif "NDF" in chi2options:
        plt.ylabel(f'Chi2/NDF')
        plt.title(f'TProfile Chi2/NDF:"{chi2options}" values by hist')
    else:
        plt.ylabel(f'Chi2 Value')
        plt.title(f'TProfile Chi2 Values:"{chi2options}" values by hist')
    plt.xlabel('Hist Name')
    plt.xticks(rotation=90)
    plt.grid(True)
    plt.show()
...
def print_chi2_by_thresh(df, threshold):
    # TODO: handle args.perndf, args.p, (neither), and other configurations (will output different column names chi2/ndf, chi2, p_value)
    
    print("Gathering chi2_vals >= threshold...")
    
    df_chi2_vals_above_thresh = df[df['output_vals']>=threshold]
    df_chi2_vals_above_thresh = df_chi2_vals_above_thresh[['f_name', 'output_vals']]
        
    print(f"--- Results (vals >= {threshold} threshold) ---")
    for i in range(df_chi2_vals_above_thresh.shape[0]):
        print(f"[ Entry {i} ]")
        print(f'  Histogram: {df_chi2_vals_above_thresh.iloc[i, 0]}') # 0 = 'f_name'
        # We can do this without taking args as input because we have declared it as a global in __main__
        if args.perndf:
            print(f'  Chi2/NDF Value: {df_chi2_vals_above_thresh.iloc[i, -1]}') # -1 = 'output_vals'
        elif args.p:
            print(f'  P-Value: {df_chi2_vals_above_thresh.iloc[i, -1]}') # -1 = 'output_vals'
        else:
            print(f'  Chi2 Value: {df_chi2_vals_above_thresh.iloc[i, -1]}') # -1 = 'output_vals'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        Required:
          - Singularity, lxplus/locally: Running with the singularity avd-tool-container.sif simply requires executing ". run_avd_tool_env.sh" in the terminal. (locally requires singularity installed/compiled)
          - From Docker Container: Running with docker requires: "docker pull rdccdr/dockerhub:avd-tool-env", "docker run -it rdccdr/dockerhub:avd-tool-env" (make sure daemon is installed/running)
          - Local, no container: Running the script without a container requires (ROOT, matplotlib, pandas, seaborn). Recommended to build the env using conda-forge.
          - For lxplus: This script requires X11 installed on Linux, X11Server on Windows, or XQuartz on Mac to display on your local machine after running the command on lxplus.
        ...
        - python atlas_validation_tool.py --file /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TH1 --folders Tau egamma --mode diff --hname run_472943/CaloMonitoring/TileCellMon_NoTrigSel/General/Summary/TIME_execute
        - python atlas_validation_tool.py --file data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TH1 --folders Tau egamma --mode diff --hname run_472943/CaloMonitoring/TileCellMon_NoTrigSel/General/Summary/TIME_execute
        - python atlas_validation_tool.py --file /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TH1 --folders CaloMonitoring egamma --mode chi2 --norm occ_area --perndf
        - python atlas_validation_tool.py --file data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TH1 --folders CaloMonitoring egamma --mode chi2 --norm occ_area --perndf
        - python atlas_validation_tool.py --file data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TH1 --mode chi2 --folders Tau CaloMonitoring --perndf --print 10
        - python atlas_validation_tool.py --file data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --htype TProfile --mode chi2 --folders Tau CaloMonitoring --perndf --print 0.0
        (When running the script on LXPLUS, assumed to be in the same directory as the script OR in a cernbox eos location)
        - python atlas_validation_tool.py --file /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref /eos/home-c/crandazz/SWAN_projects/ATLAS_DQ_Dashboard/data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --folders Tau --htype TH2 --mode diff --hname run_472943/Tau/Calo/Tau_Calo_centFracVsLB
        - python atlas_validation_tool.py --file data24_13p6TeV.00472943.physics_Main.merge.HIST.f1442_h464._0001.1 --ref data24_13p6TeV.00472943.physics_Main.merge.HIST.r15810_p6305.root --folders Tau --htype TH2 --mode diff --hname run_472943/Tau/Calo/Tau_Calo_centFracVsLB
    ...
    parser.add_argument('--print', type=float, help='Optional for chi2 mode only: Prints out the chi2_values >= threshold, use "--print <threshold>(float)".')
    ...
    if args.print and args.mode !='chi2':
        parser.error('"--mode chi2" must be set to use the optional "--print <threshold>" parameter (float)')
    ...
    if not args.mode=='diff':
        ...
        if  args.htype == "TH1":
            ...
            if args.mode == 'dist':
                print("Plotting TH1 histograms Dist...")
                plot_dist_th1(df_th1, chi2options)
            
            elif args.mode == 'chi2' and args.print is not None:
                print("Printing TH1 histograms Output Vals...")
                print_chi2_by_thresh(df_th1, args.print)
            
            elif args.mode == 'chi2':
                print("Plotting TH1 histograms Output Vals...")
            ...
            if args.mode == 'dist':
                print("Plotting TProfile histograms Dist...(erroneous points removed)")
                plot_dist_th2(df_th2, chi2options)
                
            elif args.mode == 'chi2' and args.print is not None:
                print("Printing Chi2 histograms Output Vals... (erroneous points removed)")
                print_chi2_by_thresh(df_th2, args.print)
            ...
            df_tp = df[df['f_type']=='TProfile']
            if args.norm == 'occ_area':
                print("Normalizing TProfile Chi2 vals to occupancy area... (erroneous points removed)")
                output_vals_normed = integral_normalize_histogram(df_tp)
                df_tp.loc[:,'output_vals'] = output_vals_normed
                
            
            if args.mode == 'dist':
                print("Plotting TProfile histograms Dist...")
                plot_dist_tps(df_tp, chi2options)
                
            elif args.mode == 'chi2' and args.print is not None:
                print("Printing TProfile histograms Output Vals...")
                print_chi2_by_thresh(df_tp, args.print)
                
            elif args.mode == 'chi2':
                print("Plotting TProfile histograms Output Vals...")
